...
@Component({
	...
	imports: [ThemeHoverDirective,MonthPipe,CommonModule]
})
export class NgPersianDatepickerComponent
	implements OnInit, OnDestroy, OnChanges
{
	constructor(private elementRef: ElementRef<HTMLElement | null>,@Inject(PLATFORM_ID) private platformId: Object) {
		this.setToday();
	}
	ngOnChanges(changes: SimpleChanges): void {
		for (let propName in changes) {
			if (propName === 'isView' && changes['isView'].previousValue != undefined) {
				if (this.uiIsVisible) this.setUiIsVisible(false);
				else {
	...
	set _ngModal(value: FormControlName | undefined) {
		this.setFormControl(value?.control);
	}
	...
	setDateInitValue(dateValue: string | number | null | undefined): void {
		...
		this.dateOnInit.next({
			...
			gregorian: String(
				this.selectedDate.gregorian(this.dateGregorianFormat),
			),
	...
	setViewDate(): void {
		if (!this.dateValueDefined()) {
			...
			this.viewDate =
				this.dateMax &&
				this.selectedDate!.valueOf() > this.dateMax.valueOf()
					? Jalali.timestamp(this.dateMax, false)
	...
	setViewDateTitle(): void {
		...
		switch (this.viewModes[this.viewModeIndex]) {
			case 'day':
				this.viewDateTitle =
					months[this.viewDate.getMonth()] + ' ' + year.toString();
				break;
			case 'month':
	...
	scrollIntoActiveTime(): void {
		...
		setTimeout(() => {
			// Hour
			const activeHour = this.elementRef.nativeElement?.querySelector(
				'.time-col.hour-col .dp-btn.selected',
			);
			if (activeHour) activeHour.scrollIntoView({ block: 'center' });
			// Minute
			const activeMinute = this.elementRef.nativeElement?.querySelector(
				'.time-col.minute-col .dp-btn.selected',
			);
			if (activeMinute) activeMinute.scrollIntoView({ block: 'center' });
			// Second
			const activeSecond = this.elementRef.nativeElement?.querySelector(
				'.time-col.second-col .dp-btn.selected',
			);
